...
# -3 - Goal point
map_size = [60, 60]
_map_, start, goal = generateMap2d(map_size)
plt.clf()
plt.imshow(_map_)
plt.show()
...
# Enter name of algo to use ('random', 'bfs', 'dfs', 'gbfs', 'astar')
algorithm = 'random'

example_solved_path = search_algorithm.search(_map_, start, goal, algorithm)

if example_solved_path:
    print(f"Start: {start} -- Goal: {goal}")
    print(f"Path found using {algorithm}, cost of {example_solved_path[1]}")
    for value in example_solved_path[0]:
        print(value)
    plotMap(example_solved_map, example_solved_path[0])
else:
