...
def setMotorSpeeds(motorSpeed):
    try:
        ...
        vrep.simxSetJointTargetVelocity(robot.clientID, robot.leftMotorHandle, motorSpeed.get('speedLeft', 0),
                                        vrep.simx_opmode_oneshot)
        vrep.simxSetJointTargetVelocity(robot.clientID, robot.rightMotorHandle, motorSpeed.get('speedRight', 0),
                                        vrep.simx_opmode_oneshot)
    finally:
...
def robotDirection():
    retCode, robotOrientation = vrep.simxGetObjectOrientation(robot.clientID, robot.pioneerRobotHandle, -1,
...
def init():
    ...
    if clientID != -1:
        ...
        if res == vrep.simx_return_ok:  # Remote function call succeeded
            ...
            ret_lm, leftMotorHandle = vrep.simxGetObjectHandle(clientID, 'Pioneer_p3dx_leftMotor',
                                                               vrep.simx_opmode_oneshot_wait)
            ret_rm, rightMotorHandle = vrep.simxGetObjectHandle(clientID, 'Pioneer_p3dx_rightMotor',
                                                                vrep.simx_opmode_oneshot_wait)
            ret_pr, pioneerRobotHandle = vrep.simxGetObjectHandle(clientID, 'Pioneer_p3dx',
                                                                  vrep.simx_opmode_oneshot_wait)
            ret_sl, ultraSonicSensorLeft = vrep.simxGetObjectHandle(clientID, 'Pioneer_p3dx_ultrasonicSensor3',
                                                                    vrep.simx_opmode_oneshot_wait)
            ret_sr, ultraSonicSensorRight = vrep.simxGetObjectHandle(clientID, 'Pioneer_p3dx_ultrasonicSensor5',
                                                                     vrep.simx_opmode_oneshot_wait)
            blockHandleArray = []
            for i_block in range(12):
                ...
                if i_block > 6:
                    rand_loc = [random.random() * 6 - 1.5, random.random() * 7 - 2.5,
                                0.0537]  # x[-1.5,4.5] y[-2.5,-4.5]
...
def findEnergyBlocks():
    res = []
    retCode, robotPos = vrep.simxGetObjectPosition(robot.clientID, robot.pioneerRobotHandle, -1,
                                                   vrep.simx_opmode_oneshot_wait)
...
def collectNearestBlock():
    ...
    if distance <= 0.5:
        ...
        return ('Energy collected :)', (blockName, getSimulationTime()))
    return ('No blocks nearby :(', False)
